...
#    model, optimizer = get_trained_model_optimizer(args, device, train_loader, val_loader, criterion)
    ...
    model = VGG(my_shape=my_shape, depth=16).to(device)
    ...
    if args.pruner == 'L1FilterPruner':
        ...
        pruner = NetAdaptPruner(model, config_list, short_term_fine_tuner=short_term_fine_tuner, evaluator=evaluator, val_loader=val_loader, num=num, dummy_input=dummy_input, my_shape=my_shape, criterion=criterion, base_algo=args.base_algo, experiment_data_dir=args.experiment_data_dir)
