...
#    my_shape = cPickle.load(open(os.path.join('/github/evta2/output', str(num), 'my_shape.p'),'rb'))
    model = VGG(depth=num).to(device)
    summary(model, (3, 32, 32)) ##
    model.load_state_dict(torch.load(os.path.join('/github/evta2/output', str(num), 'model_trained.pth')))
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9, weight_decay=5e-4) # lr=1e-4
...
#            train(args, model, device, train_loader, criterion, optimizer, epoch)
    ...
    if args.pruner == 'L1FilterPruner':
        ...
        pruner = NetAdaptPruner(model, config_list, short_term_fine_tuner=short_term_fine_tuner, evaluator=evaluator, val_loader=val_loader, num=num, dummy_input=dummy_input, criterion=criterion, base_algo=args.base_algo, experiment_data_dir=args.experiment_data_dir)
    ...
    if args.speed_up:
        if args.pruner != 'AutoCompressPruner':
            ...
            torch.save(model.state_dict(), os.path.join(args.experiment_data_dir, str(num), 'model_speed_up.pth'))
            print('Speed up model saved to %s' % args.experiment_data_dir)
        flops, params, _ = count_flops_params(model, get_input_size(args.dataset))
        result['flops']['speedup'] = flops
        result['params']['speedup'] = params
    file_object = open('./record_tvm.txt', 'a')
    file_object.write('Before long-term_fine_tune: {}\n'.format(datetime.now()))
    file_object.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment-data-dir', type=str, default='/github/evta2/output', #'./experiment_data',
